...
function retake() {
  ...
  console.log(`original:'${remainCount}`)
  remainCount = remainCount-1;
  console.log(`afterdeduction:'${remainCount}`)
  if (remainCount <= 0) {
    $('.btn-retake').addClass('btn-disabled');
  }
}
...
function showQuestion() {
  if (remainCount === 2){
    showScene('#scene-question-1');}
  else if(remainCount === 1){
    showScene('#scene-question-1-2');}
  else if(remainCount === 0) {
    showScene('#scene-question-1-3');
  }
  }
...
$(document).ready(function () {
  ...
  readJson('data/questions.json', function(questions) {
    ...
    readJson('data/choices.json', function(choices) {
      optionList = choices;
      loadQuestion();
    });
  });
